...
# ----------------------------------------------------------------------
SAFE_ROOTS = [Path.home()]   # can be extended

def is_safe_path(target: Union[str, Path]) -> bool:
    """Return True if the target lies inside one of the allowed SAFE_ROOTS."""
    try:
        resolved = Path(target).resolve()
        return any(resolved == root.resolve() or resolved.is_relative_to(root.resolve())
