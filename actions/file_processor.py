...
# ─────────────────────────────────────────────────────────────────────────────
# Google GenAI SDK migration helper
# Replaces legacy Gemini GenerativeModel usage.
...
class _GeminiModel:
    """Small compatibility wrapper around the new google-genai SDK."""

    def __init__(self, model_name: str | None = None, system_instruction: str | None = None, **kwargs):
...
import subprocess
import tempfile
from core.utils import get_api_key, format_size, log
from pathlib import Path
from datetime import datetime
    # migrated: using _GeminiModel / google-genai
def _gemini_client():

    return _GeminiModel("gemini-2.5-flash")
...
def _load_audio_segment():
    try:
        import importlib
        return importlib.import_module("pydub").AudioSegment
    except ImportError:
        return None
...
def _process_image(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    if action == "compress":
        ...
        try:
            ...
            before = file_size_str(path)
            after  = file_size_str(out)
    ...
    if action == "info":
        try:
            ...
            return (f"Image info: {img.format}, {img.size[0]}x{img.size[1]}px, "
                    f"mode: {img.mode}, size: {file_size_str(path)}")
        except Exception as e:
...
def _process_pdf(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    def _extract_pdf_text(max_chars=50000) -> str:
        text = ""
        try:
            pdfplumber = __import__("importlib").import_module("pdfplumber")
            with pdfplumber.open(path) as pdf:
                for page in pdf.pages:
                    text += (page.extract_text() or "") + "\n"
        except ImportError:
            try:
                PyPDF2 = __import__("importlib").import_module("PyPDF2")
    ...
    if action == "info":
        try:
            ...
            with pdfplumber.open(path) as pdf:
                pages = len(pdf.pages)
            return f"PDF: {pages} pages, size: {file_size_str(path)}"
        except Exception:
            return f"PDF size: {file_size_str(path)}"
...
def _process_data(path: Path, file_type: str, action: str,
                  params: dict, speak=None) -> str:
    try:
        pd = __import__("importlib").import_module("pandas")
    except ImportError:
...
def _process_json(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    if action == "validate":
        return f"Valid JSON. Type: {type(data).__name__}, size: {file_size_str(path)}"
...
def _process_code(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    if action == "info":
        lines = content.count("\n")
        words = len(content.split())
        return f"Code file: {lines} lines, {words} words, {file_size_str(path)}"
    ...
    if action == "info":
        try:
            ...
            return (f"Audio: {mins}m {secs}s, "
                    ...
                    f"{file_size_str(path)}")
        except ImportError:
            return f"Audio file: {file_size_str(path)} (install pydub for more info)"
        except Exception as e:
            return f"Info failed: {e}"
...
def _process_video(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    if action == "info":
        try:
            ...
            return f"Video file: {file_size_str(path)}"
