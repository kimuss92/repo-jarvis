...
def _send_whatsapp(receiver: str, message: str) -> str:
    ...
    brings it to the foreground, isolates the contact, and dispatches the message.
    """
    import win32gui
    import win32con
    import win32process
    ...
    # Save original foreground window frame and mouse coordinates
    ...
    if whatsapp_hwnd != 0:
        # Instance is already running -> restore it and pull it straight to the front
        ...
        # No running instance found -> perform a clean startup launch sequence
        if not _open_app("WhatsApp Web"):
            return "Could not open your WhatsApp Web shortcut, sir."
        time.sleep(5.5)  # Extended buffer to allow the web app engine to fully load and authenticate
    ...
    time.sleep(2.0)  # Wait for the network search layout to populate matching contacts

    # 5. Lock onto the filtered user chat panel card
    pyautogui.press("enter")
    time.sleep(1.2)  # CRITICAL: Buffer to wait for the chat message container pane to render completely

    # 6. Type the message string payload and fire it off
    _paste_text(message)
    time.sleep(0.3)
    pyautogui.press("enter")
    time.sleep(0.3)
    ...
    return f"Message sent to {receiver} via WhatsApp Web shortcut, sir."
