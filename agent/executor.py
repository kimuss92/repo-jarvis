...
def _run_generated_code(description: str, speak: Callable | None = None) -> str:
    if os.environ.get('JARVIS_ALLOW_GENERATED_CODE', '').lower() not in ('1', 'true', 'yes'):
        raise RuntimeError('Generated code execution is disabled. Use code_helper/dev_agent or set JARVIS_ALLOW_GENERATED_CODE=1 intentionally.')
...
def _call_tool(tool: str, parameters: dict, speak: Callable | None) -> str:
    original_tool, original_parameters = tool, dict(parameters or {})
    try:
        tool, parameters = route_tool_call(tool, parameters or {})
        if (tool, parameters) != (original_tool, original_parameters):
            print(f"[Executor] 🧭 routed {original_tool} {original_parameters} → {tool} {parameters}")
    except Exception as route_error:
        print(f"[Executor] ⚠️ routing layer skipped: {route_error}")
